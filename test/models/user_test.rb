...
class UserTest < ActiveSupport::TestCase
  ...
  test "user email can be registered globally or account specific" do
    attributes = {name: "Test", email: "new@example.org", password: "password", password_confirmation: "password", terms_of_service: true}
    assert_difference "User.count", 2 do
      User.create!(attributes)
      User.create!(attributes.merge(account: accounts(:marketplace)))
    end
  end
end
