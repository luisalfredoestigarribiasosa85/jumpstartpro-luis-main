...
class User < ApplicationRecord
  ...
  # Include default devise modules. Others available are: :lockable, :timeoutable, andle :trackable
  devise :database_authenticatable, :registerable, :recoverable, :rememberable, :confirmable, :omniauthable
  ...
  # Associations
  belongs_to :account, optional: true # Separates marketplace users if enabled
  ...
  validates :email,
    presence: true,
    uniqueness: {scope: :account_id, allow_blank: true, if: :email_changed?},
  ...
  # Look up user with scoping for current merchant
  def self.find_for_authentication(conditions)
    find_by(conditions.merge(account: Current.merchant))
  end
end
