module AccountsHelper
  ...
  # Returns a hash of options for URL helpers
  #
  # record_url(@record, account_url_options)
  def account_url_options(account = Current.merchant)
    return {} unless account && (account.domain || account.subdomain)
    account.domain? ? {domain: account.domain} : {subdomain: account.subdomain}
  end
